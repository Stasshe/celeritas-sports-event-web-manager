...
### システム機能
- **自動保存**: 編集内容をdebounceで自動保存、手動保存は必要な箇所のみ
...
│   └── [アイコン画像]       # 各サイズのアイコン
├── src/                    # ソースコード
│   ├── general/             # 一般ユーザー向け(公開画面)
│   │   ├── pages/            # Home/Sport/Login/NotFound/ScoreboardDetails/ClassSchedule
│   │   └── components/       # PublicLayout, schedule/scoreboard/sports表示用
│   ├── admin/                # 管理画面
│   │   ├── pages/            # 管理者向け全ページ
│   │   ├── components/       # AdminLayout, dialogs/scheduling/scoreboard/scoring
│   │   └── context/          # AdminLayoutContext(保存パイプライン一元管理)
│   ├── common/                # general/adminどちらからも使う共有機能(トーナメント表)
│   ├── config/               # 設定ファイル(Firebase初期化)
│   ├── contexts/             # 全体共通Reactコンテキスト(Auth, Theme)
│   ├── hooks/                # カスタムフック(useDatabase等)
│   ├── types/                # TypeScript型定義
│   ├── utils/                # ユーティリティ関数
│   │   └── export/            # エクスポート機能
│   ├── App.tsx              # アプリケーションルート
...
A: 競合解決の仕組みはない。Firebase RTDBへの直接書き込みで最後に保存した内容が勝つ。複数人で同一イベント/競技を同時編集する運用は避けること。
