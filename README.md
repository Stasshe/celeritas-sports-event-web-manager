...
![バージョン](https://img.shields.io/github/package-json/v/Stasshe/celeritas-sports-event-web-manager)
![ライセンス](https://img.shields.io/badge/license-GPL-green)
![言語](https://img.shields.io/badge/TypeScript-7.0-blue)
![フレームワーク](https://img.shields.io/badge/React-19.2-61dafb)
...
### イベント管理
- **イベントの作成・編集・削除**: 日程、場所、改行可能な説明などの詳細情報を含むイベントの管理
...
### 競技管理
- **多様な競技形式のサポート**:
  - トーナメント形式（メイン・負け側それぞれの3位決定戦、メイン3位決定戦の参加者を除く1回戦または2回戦敗者による負け側トーナメント）
...
- **詳細なスケジュール設定**:
  ...
  - 時間枠の管理（試合、休憩、昼食など）
  - 準備時間・片付け時間の設定
- **手動編集**: 進行表内で任意の対戦名を持つ行を追加し、ドラッグまたは上下ボタンで時刻を保持する入れ替えと時刻ごとの入れ替え
- **同時開催試合の管理**: 複数コートでの同時進行試合の表示と管理
- **2コート表示**: 管理・公開タイムラインとも、画面幅にかかわらず同時刻の2コートを横並びで表示
- **試合時間の自動計算**: 試合数と利用可能時間から最適な時間配分を算出
- **進行差の表示**: 遅延は正数、前倒しは負数で表示
- **タイムライン表示**: 全競技を統合したイベント全体のタイムライン
- **クラス別スケジュール**: トーナメントのメイン・負け側を辿り、確定試合と出場可能性のある試合を表示
...
- **レスポンシブなトーナメント表**: PCは全体を静的表示し、スマートフォンは表示枠内のドラッグ移動・ピンチ拡大縮小に対応
...
管理スケジュールは、デスクトップで生成条件と進行表を並べ、モバイルでは生成条件を折りたたんで進行表を優先します。直接編集は別画面を開かず進行表内で切り替え、専用ハンドルのドラッグ、キーボード、上下ボタンで同じ並べ替えを行えます。編集中の変更は作業コピーへ保持し、Ctrl+Z / Ctrl+Yで保存前の編集を戻す・やり直すことができます。保存で一括反映し、キャンセルで破棄します。

### システム機能
- **自動保存**: 管理画面とモーダルの編集をscope単位でデバウンス保存し、競合しない直列writeと保存状態を共通管理
- **オフライン編集**: インターネット接続がない環境での編集とオンライン復帰時の同期
- **変更検知**: 未保存の変更がある場合の通知とページ遷移時の警告(admin全体で共通の保存状態管理)
- **障害情報の共有**: 起動・描画エラー画面に詳細情報と一括コピーボタンを表示
...
### フロントエンド
- **React 19**: UIコンポーネントの構築とレンダリング
- **TypeScript 7**: 型安全なコード開発
- **Material UI 9**: UIコンポーネントとテーマ
- **React Router 7**: SPA内の画面遷移とルーティング
- **styled-components 6**: トーナメント描画のスタイル基盤
- **@cm3tahkuh/react-tournament-brackets 1.1**: トーナメント表描画
- **Framer Motion 12**: アニメーション効果
...
### 開発・デプロイツール
- **Vite 8**: 開発サーバーと本番ビルド
- **React Scan**: 開発環境でのReactレンダリング性能の可視化
...
3. 画面右下のReact Scanツールバーからレンダリング性能を確認できます。React Scanは本番ビルドでは無効です。

4. 初回ログイン時に管理者アカウントを作成します。
...
開発中の監視実行は `pnpm test:watch`。トーナメント構造、シード、3位決定戦、リーグ順位参照、将来試合のクラス別抽出、1/2コート配置、時間刻み、休憩境界、順序維持リスケをVitestで検証する。
...
GitHub Actionsの`Create Release`を手動実行し、`patch`、`minor`、`major`からversion更新幅を選ぶ。workflowは依存関係cacheを使ってtestとbuildを検証し、`package.json`のversion更新、`tree.sh`による`tree.txt`再生成、release commitとtag、GitHub Release作成を順に実行する。
...
- **3位決定戦**: メインと負け側で個別に設定し、各準決勝敗者同士の試合を対応表の直下へ表示
- **負け側トーナメント**: メイン3位決定戦の参加者を除く指定ラウンドの敗者で構成
- **ブラケット操作**: PCは静的表示、スマートフォンは背景のドラッグ移動・ピンチ拡大縮小と試合カードのタップ編集に対応
