...
管理スケジュールは、デスクトップで生成条件と進行表を並べ、モバイルでは生成条件を折りたたんで進行表を優先します。直接編集は別画面を開かず進行表内で切り替え、専用ハンドルのドラッグ、キーボード、上下ボタンで同じ並べ替えを行えます。編集中の変更は作業コピーへ保持し、保存で一括反映、キャンセルで破棄します。
