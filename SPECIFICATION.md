...
メイン・負け側のトーナメント表は表示領域の幅へ追従する。`sm`以上は表全体を静的表示し、パン・ズームを持たない。`sm`未満だけ固定高viewportとし、背景のタッチドラッグで移動、ピンチで拡大縮小する。試合カードのタップは移動操作より優先し、管理画面では試合結果編集を開く。操作はviewport内で完結させ、page全体へ横スクロールを発生させない。
