...
管理スケジュールは、広幅で生成条件と進行表を並置し、狭幅では生成条件を折りたたんで進行表を主領域にする。表示と直接編集は同じ進行表領域で切り替える。並べ替えはマウス・タッチ・キーボード対応の専用ハンドルを正本とし、上下移動ボタンも常設する。タッチ操作はハンドルの長押しで開始し、行内フォームとページの縦スクロールを妨げない。
