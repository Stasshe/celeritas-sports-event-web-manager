...
メイン・負け側のトーナメント表は表示領域の幅へ追従する。`sm`以上は表全体を静的表示し、パン・ズームを持たない。`sm`未満だけ固定高viewportとし、タッチドラッグで移動、ピンチで拡大縮小する。公開画面は試合カード上からも移動を開始できる。管理画面は試合カードのタップを移動操作より優先し、試合結果編集を開く。操作はviewport内で完結させ、page全体へ横スクロールを発生させない。
...
管理スケジュールは、広幅で生成条件と進行表を並置し、狭幅では生成条件を折りたたんで進行表を主領域にする。表示と直接編集は同じ進行表領域で切り替える。編集内容は作業コピーへ保持し、保存時だけ永続データへ一括反映、キャンセル時は破棄する。Undo/Redoは直接編集中の作業コピーだけを対象とし、Ctrl+Z / Ctrl+Yおよび操作ボタンから実行する。履歴は編集開始から最大20件保持し、保存後やブラウザストレージへ持ち越さない。並べ替えはマウス・タッチ・キーボード対応の専用ハンドルを正本とし、上下移動ボタンも常設する。タッチ操作はハンドルの長押しで開始し、行内フォームとページの縦スクロールを妨げない。

管理・公開の競技タイムラインは、2コート設定時に全画面幅で時刻・コート1・コート2を同一行へ表示する。全コート共通枠は2コート列を横断する。狭幅でも片方のコートを省略または縦積みにせず、ページ横スクロールを発生させない。
